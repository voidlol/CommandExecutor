...
public class RateCommandParser implements CommandParser {
    ...
    public Command parse(String[] command) {
        ...
        PartValidationDto<AlgorithmType> algorithm = parseAlgorithm(arguments1);
    ...
    private void checkArguments(String[] arguments) {
        if (arguments.length < MINIMUM_ARGUMENTS) {
            throw new RuntimeException(Error.INVALID_ARGUMENTS.getError());
        }
    }

    private void checkErrors(PartValidationDto<CurrencyType> currencyType,
                             PartValidationDto<AlgorithmType> algorithm,
    ...
    private PartValidationDto<AlgorithmType> parseAlgorithm(List<String> arguments) {
        int indexOfAlgorithmKey = arguments.indexOf(ALGORITHM_KEY);
        if (indexOfAlgorithmKey == -1) {
            return new PartValidationDto<>(AlgorithmType.STRAIGHT, null);
        }
        if (indexOfAlgorithmKey == arguments.size()) {
            return new PartValidationDto<>(null, Error.INVALID_ALGORITHM.getError());
        }
        return AlgorithmType.findByName(arguments.get(indexOfAlgorithmKey + 1))
